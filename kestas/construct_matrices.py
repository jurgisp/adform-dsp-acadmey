...
def generate (url = '', bigram_regex='', n1 = 0, url_path=0, get=0):
    url = re.sub(r'([/?]*)$', '', url)
...
def gen_data (classifier, regex='', n1 = 0, url_path=0, get = 0, add_adv = 0, lower_pop_bound=0):
    train, test = [], []
    features = Counter()
    ...
    for id, label, history in produrator(classifier, adv_url = add_adv):
        ...
        for url in history:
            for u in generate(url, regex, n1, url_path):
                dataPoint[2].add(u)
                features[u] += 1
